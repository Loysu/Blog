...
class RegistrationView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_user.save()
            Profile.objects.create(user=new_user, slug=slugify(new_user.username))
            user = authenticate(
                username=form.cleaned_data["username"],
                password=form.cleaned_data["password"],
            )
...
class ProfileView(DefaultContextMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(self.request, **kwargs)
        context["user"] = self.request.user
        context["articles"] = Profile.objects.get(
            user=context["main_profile"].user
...
class EditProfileView(View):
    ...
    def get(self, *args, **kwargs):
        if isinstance(self.request.user, AnonymousUser):
            ...
            return redirect(reverse("post:base"))
        elif Profile.objects.get(user=self.request.user) != Profile.objects.get(
            slug=kwargs["slug"]
        ):
            messages.warning(
                self.request, "Вы можете редактировать только свой профиль!"
            )
            return redirect(reverse("post:base"))
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(self.request, "Ваш профиль успешно обновлен с:")
            user = authenticate(
                username=form.cleaned_data["username"],
                password=form.cleaned_data["password"],
            )
...
class ChangeProfilePictureView(generic.UpdateView):
    ...
    def get(self, *args, **kwargs):
        ...
        ) != Profile.objects.get(slug=kwargs["slug"]):
            messages.warning(self.request, "У вас недостаточно прав!")
            return redirect(reverse("post:base"))
...
class CreatePostView(DefaultContextMixin, generic.CreateView):
    ...
    def get(self, *args, **kwargs):
        if (
            ...
            or not Profile.objects.get(user=self.request.user).author
        ):
            messages.warning(self.request, "У вас недостаточно прав!")
            return redirect(reverse("post:base"))
        return super().get(*args, **kwargs)
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            "Ваш пост успешно создан с:\nАдминистрация проверит его и опубликует!",
        )
        return redirect(
            reverse(
                "post:profile", kwargs={"slug": slugify(self.request.user.username)}
            )
        )
...
class EditPostView(DefaultContextMixin, View):
    ...
    def get(self, *args, **kwargs):
        if (
            isinstance(self.request.user, AnonymousUser)
            or Post.objects.get(slug=kwargs["slug"]).author.user != self.request.user
        ):
            messages.warning(self.request, "У вас недостаточно прав!")
            return redirect(reverse("post:base"))
